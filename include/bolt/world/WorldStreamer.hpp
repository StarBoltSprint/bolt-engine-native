...
struct WorldChunk {
  ...
  std::vector<FoliageInstance> flying; // FlyingGenerator air layer
...
public:
  ...
  /** Returns true if any chunk was loaded/unloaded/resampled (GPU re-upload needed). */
  bool update(const SprintCore& sprint, const SpawnRules& rules, const HeightField& height,
              const SpawnBudgets& budgets, const VegetationSpawner& veg,
              const DetailSpawner& detail, const FlyingGenerator& flying,
  ...
  void gatherFlying(std::vector<FoliageInstance>& out) const;
